...
class GameState:
    def __init__(self, k, n, moi, mode):
        ...
        # (in the discard pile and hands of OTHER players)
        # we init for each player, the seen cards of each combinasion to 0
        #
        # Note: cards on the played piles are NOT counted here, on purpose.
        # Criticality is always gated by `piles[c] <= v-1` (card still needed),
        # so a card whose value is already on its pile is never critical and a
        # played copy can never change that verdict -> counting piles is moot.
        self.seen_cards = {
...
class AI:
    ...
    # For mode 4 to 7 (and 8)
    def smart_action(self):
        return (self.rule_protect_critical()  # rule 2ter (mode 8) - top priority
                or self.rule_special_play()   # rule 1bis (mode 7)
                or self.rule_play()           # rule 1 (mode 4)
                or self.rule_hint()           # rule 2 (mode 4) / 2bis (mode 7)
                or self.rule_discard())       # rule 3 (mode 4) / 3bis (mode 6) / 4 (mode 8)


    # Rule 2ter (mode 8)
    # extension.md: protect a critical card of another player BEFORE even looking
    # for a directly stackable card of our own. We scan ALL players/positions for
    # a critical, playable, not-yet-special card and send it a (special) hint.
    def rule_protect_critical(self):
        g = self.game
        if g.mode < 8 or g.tokens <= 0:
            return None
        for j in range(1, g.n + 1):
            if j == g.moi:  # we cannot see/assess our own cards' identity
                continue
            player = g.players[j]
            for p in player.positions():
                card = player.cards[p]
                if not card.visible or not g.is_playable(card.value, card.color):
                    continue
                kn = player.knowledge[p]
                # skip if already known completely or already incited to play it
                if kn.complete or kn.special:
                    continue
                if self.is_critical_hint(card.value, card.color):
                    # prefer the value hint, fall back to color hint
                    if kn.value is None:
    ...
    def rule_discard(self):
        ...
        for p in me.positions():
            ...
            if d < best_date or (d == best_date and p < best_p):
                # Rule 4 (mode 8): never pick a known critical card if avoidable
        ...
        if (g.mode >= 8 and best_p == -1):
            ...
            # so we take the one with the lowest date (fall back to rule 3bis)
            for p in me.positions():
                d = me.knowledge[p].date
                if d < best_date or (d == best_date and p < best_p):
                    best_p, best_date = p, d
    ...
    # This is called before attempting to discard one of OUR cards (v,c).
    # `seen` counts the OTHER copies of (v,c) we have located (other players'
    # hands + our discards); our own card is not in `seen`, hence the total-1
    # threshold. The card is critical if it is the last located copy AND it is
    # still needed (its value not yet on the pile).
    # Total number of cards of any (v,c): 3 if v=1, 1 if v=k, else 2.
    ...
    # This is called before giving a special hint about ANOTHER player's card (v,c).
    # Here `seen` INCLUDES that very card (counted when it was dealt to its owner),
    # so the other copies number `seen - 1`; requiring `seen >= total` is exactly
    # the same "last surviving useful copy" condition as is_critical, shifted by one.
